...
const blockItem = document.querySelectorAll('.block-item');

const url = location.href;
const linkPrivacy = document.querySelector('.sidebar-menu-link.privacy a');

if (url == 'https://sidepieceapp.com/privacy.php') {
    linkPrivacy.classList.add('active');
}

function resetClassBlock() {
    menuItemContent.forEach(item => item.classList.remove('active'));
}

sidebarItem.forEach((item) => {
    if (currentUrl == item.dataset.link) {
        resetClassBlock();
        item.classList.add('active');
        blockItem.forEach((el) => {
            if (item.dataset.link == el.dataset.block) {
...
if (textArea) {
    ...
    document.getElementById('report_second').innerHTML = '';
}
...
function removeClassTermsAndPrivacy() {
    resetClassSelect();
    menuItemContent.forEach(item => item.classList.remove('active'));
...
function selectTabNav() {
    ...
    tabName = this.getAttribute("data-link");
...
listNavItems.forEach((item) => {
    ...
    item.addEventListener('click', (i) => {
        ...
        if (plus && window.innerWidth > 1300) {
...
function chooseTabName(tabName) {
    ...
    chooseHelpContentMobile.forEach((item) => {
        ...
        if (checkItemMobile == tabName && window.innerWidth < 1300) {
            item.classList.toggle("active");
        } else {
            item.classList.remove("active");
        }
...
function resetClassSelect() {
    containerHelpItem.classList.remove('no-active');
    aboutChoouseTab.forEach(item => item.classList.remove('active'));
    burger.classList.remove('change');
    sidebarMenu.classList.remove('active');
    body.classList.remove('active');
    ...
    $('#submit-support').click(function () {
        ...
        if (name == '' || email == '') {
            if (textInfo > 0) {
                ...
                $('.response-support').html('<span class="text-error">Please required name or email fields.</span>');
            }
